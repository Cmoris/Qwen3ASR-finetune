...
def parse_args():
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="Number of parallel inference shards.",
    )
...
def main():
    ...
    num_shards = int(data_args.num_shards)
    shard_id = int(data_args.shard_id)
    ...
    print(f"[init] shard_id={shard_id}, num_shards={num_shards}")
    print(f"[init] model_path={ASR_MODEL_PATH}")

    asr = Qwen3ASRModel.LLM(
        model=ASR_MODEL_PATH,
        gpu_memory_utilization=data_args.gpu_memory_utilization,
        max_new_tokens=256,
...
Output the transcript in chronological order."""
    ...
    with output_path.open("w", encoding="utf-8") as fout:
        for sample_idx, data in enumerate(dataset):
            # 如果 max_samples 表示全局前 N 个样本，就把这个判断放在分片前
            if data_args.max_samples > 0 and sample_idx >= data_args.max_samples:
                break

            # sample-level data parallel
            ...
            print(
                ...
                f"sample {sample_idx}, local_processed {processed} =========="
            )
    ...
    print(f"\n[done] shard {shard_id}/{num_shards}")
